# -*- coding: utf-8 -*-
"""
Created on Sat Jun 20 11:32:25 2020
...
def multilayer_perceptron(_X, _weights, _biases):
    ...
    #Hidden layer with relu activation
	layer_2 = tf.nn.dropout(tf.nn.relu(tf.add(tf.matmul(layer_1, _weights['h2']), _biases['b2'])),1.0)
	layer_3 = tf.nn.dropout(tf.nn.relu(tf.add(tf.matmul(layer_2, _weights['h3']), _biases['b3'])),1.0)
	layer_4 = tf.nn.dropout(tf.nn.relu(tf.add(tf.matmul(layer_3, _weights['h4']), _biases['b4'])),1.0)
	return tf.matmul(layer_4, _weights['out']) + _biases['out']


print("Loading saved Weights for layer 1,2,3 ...")
...
regularizers = tf.nn.l2_loss(weights['h1']) + tf.nn.l2_loss(weights['h2']) + tf.nn.l2_loss(weights['h3']) + tf.nn.l2_loss(weights['h4']) + tf.nn.l2_loss(weights['out']) 
